...
class Weibo(object):
    ...
    def user_to_mysql(self):
        ...
        # 创建数据库
        create_database = """CREATE DATABASE IF NOT EXISTS """+self.mysql_config["db"]+""" DEFAULT
    ...
    def handle_download(self, file_type, file_dir, urls, w):
        """处理下载相关操作"""
        file_prefix = (w["created_at"][:11].replace("-", "") + "_" + str(w["id"])).replace(" ", "")
        if file_type == "img":
            if "," in urls:
                url_list = urls.split(",")
